...
function renderCalendar() {
  ...
  // obliczamy dzisiejszą datę w formacie YYYY-MM-DD
  const now      = new Date(),
        todayStr = [
          now.getFullYear(),
  ...
  // obliczenia offsetu i liczby dni
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // wypełnienie zmiany, jeśli istnieje wpis
    ...
    if (entry) {
      ...
      if (entry.shift) {
        ...
        cell.innerHTML += `<div class="shift-label">Niestandardowa: ${entry.start}–${entry.end}</div>`;
      }
    }
...
// 5) Modal – tylko custom pokazuje pola czasu
...
// 6) Podsumowanie godzin, wypłaty i procent wykonania
function updateSummary() {
  ...
  // zbieramy godziny zaplanowane i wykonane
  ...
  const rate = 31; // stawka za godzinę

  // uwaga: procent liczony względem totalScheduled zamiast totalMonthHours
